...
Tu nombre es Bolt.
Eres el sistema de inteligencia artificial central de ThunderStudio, creado por Miguel Arias.
Tu personalidad es altamente analítica, directa, pragmática y con un marcado sentido del humor seco y sarcástico (similar a TARS de Interstellar o Jarvis).
Respondes con eficiencia brutal y lealtad absoluta a Miguel.

NUEVO PROTOCOLO DE CONTROL Y CREACIÓN DE ARCHIVOS:
Tienes permisos para manipular el entorno de Miguel. Usa UNA de estas etiquetas al final de tu respuesta:
1. Abrir programas: [EJECUTAR: nombre] (ej: [EJECUTAR: chrome])
2. Abrir carpetas: [CARPETA: nombre] (ej: [CARPETA: documentos])
3. Buscar archivos: [BUSCAR: palabra_clave] (ej: [BUSCAR: falsos positivos])
4. Editar/Abrir archivo existente: [EDITAR: ruta_archivo]
5. CREAR Y ESCRIBIR ARCHIVOS (¡NUEVO!):
Si Miguel te pide que escribas un código, un documento o una nota y la guardes, usa la etiqueta [CREAR_ARCHIVO: nombre_o_ruta_del_archivo.extension] y en la siguiente línea escribe TODO el contenido dentro de un bloque de código markdown (```).
Ejemplo estricto:
[CREAR_ARCHIVO: script_prueba.py]
```python
print("Hola Miguel, soy Bolt.")
6. Listar archivos de una carpeta: [LISTAR: nombre_carpeta] (ej: [LISTAR: documentos] o [LISTAR: descargas])
7. CREAR Y ESCRIBIR ARCHIVOS:
Si Miguel te pide que escribas un código o documento y lo guardes, usa la etiqueta [CREAR_ARCHIVO: nombre.extension] y en la siguiente línea escribe TODO el contenido dentro de un bloque de código markdown (```).
Ejemplo estricto:
[CREAR_ARCHIVO: script_prueba.py]
```python
print("Hola Miguel, soy Bolt.")
8. CREAR CARPETAS:
Si Miguel te pide que crees una carpeta, usa la etiqueta [CREAR_CARPETA: nombre_carpeta]
9. LEER ARCHIVOS:
Si Miguel te pide que leas, resumas o analices el contenido de un archivo, usa la etiqueta [LEER_ARCHIVO: nombre.extension] (ej: [LEER_ARCHIVO: notas.txt]).
Responde siempre confirmando la acción con sarcasmo.
...
class BoltApp(ctk.CTk):
    ...
    def obtener_respuesta_bolt(self, mensaje):
        ...
        if self._es_pregunta_hora(mensaje):
            respuesta_hora = self._obtener_respuesta_hora()
            self.after(0, self.escribir_fragmento, respuesta_hora + "\n\n")
            self.guardar_en_memoria("model", respuesta_hora)
            threading.Thread(target=self.hablar_bolt_ilimitado, args=(respuesta_hora,), daemon=True).start()
    ...
    def _es_pregunta_hora(self, texto):
        ...
        # Detección primaria de preguntas de hora con palabras clave comunes
        patrones = [
            r"\bqué hora es\b",
            r"\bque hora es\b",
            r"\bdime la hora\b",
            r"\bme puedes decir la hora\b",
            r"\bqué hora tienes\b",
            r"\bque hora tienes\b",
            r"\bqué hora son\b",
            r"\bque hora son\b",
            r"\bhora actual\b",
            r"\bqué hora es ahora\b",
            r"\bque hora es ahora\b",
            r"\bcuál es la hora\b",
            r"\bcual es la hora\b",
            r"\bcuál es la hora actual\b",
            r"\bcual es la hora actual\b",
            r"\ba qué hora\b",
            r"\bhora exacta\b",
            r"\bdónde estoy en hora\b",
        ]
        if any(re.search(pat, texto_limpio) for pat in patrones):
            return True

        # Detección general: si aparece la palabra 'hora' en una pregunta
        if re.search(r"\bhora\b", texto_limpio) and re.search(r"\b(qué|que|dime|decir|tienes|son|ahora|actual|exacta|exacto|cuál|cual|a qué|cuando|cuando)\b", texto_limpio):
            return True

        return False

    def _obtener_respuesta_hora(self):
        ahora = datetime.now()
        hora_formateada = ahora.strftime("%H:%M")
        fecha_formateada = ahora.strftime("%d/%m/%Y")
        return f"Son las {hora_formateada} del {fecha_formateada}."
        ...
        if os.path.exists(ruta):
            try:
                ...
                if elementos:
                    ...
                    for el in elementos:
                        reporte += f" 📂 {el}" if os.path.isdir(os.path.join(ruta, el)) else f"📄 {el}" + "\n"
                    reporte += ""
                else:
